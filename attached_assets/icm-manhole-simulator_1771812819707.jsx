...
function getManholeGeometry(shaftDia, chamberDia, shaftDepth, chamberDepth) {
...
function volumeToElevation(vol, geo) {
...
// Head-driven outflow through outlet pipe (orifice equation)
// Q = Cd * A_eff * sqrt(2 * g * h)
// h = water level above pipe centerline (or soffit for submerged)
function calcOutflow(waterElev, geo, outletDia, outletInvert, gateOpen, gatePosition) {
  ...
  const effectiveArea = fullArea * gatePosition; // gate partially open
  const Q = CD * effectiveArea * Math.sqrt(2 * GRAVITY * head);
  return Q;
}

function riseRate(vol, geo, netInflow) {
...
export default function ICMManholeApp() {
  ...
  const [outletOffset, setOutletOffset] = useState(0.0); // offset above invert
  const [gateOpen, setGateOpen] = useState(true);
  const [gatePosition, setGatePosition] = useState(1.0); // 0=closed, 1=fully open
  ...
  // Simulation loop with head-driven outflow
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      // Inlet pipe Y (at invert level)
      const inletPipeY = chamberBottomY - inletPixelH * 0.9;
      // Outlet pipe Y (offset above invert)
      ...
      // Gate blade (drops down from top)
      ...
      if (waterDepthAboveInvert > 0.01) {
        ...
        // Water in outlet pipe (if flowing)
        if (currentOutflow > 0.0001 && gateOpen && gatePosition > 0) {
          ...
          // Water between chamber wall and gate
        ...
        // Turbulence at outlet if flowing
  ...
  const Slider = ({ label, value, onChange, min, max, step, color = COLORS.accent, suffix = "", disabled = false }) => (
    <div className="mb-2">
  ...
  const Stat = ({ label, value, color = COLORS.accent, alert = false }) => (
  ...
  return (
    <div className="min-h-screen p-2" style={{ background: COLORS.bg, color: COLORS.text, fontFamily: "'Segoe UI', system-ui, sans-serif" }}>
        ...
        <div className="flex flex-col xl:flex-row gap-2.5">
          ...
          <div className="w-full xl:w-60 space-y-2">
            ...
            <div className="rounded-xl p-2.5" style={{ background: COLORS.panel, border: `1px solid ${COLORS.panelBorder}` }}>
              ...
              <Slider label="Shaft Ø" value={shaftDia} onChange={(v) => setShaftDia(Math.min(v, chamberDia))} min={0.3} max={2.0} step={0.05} suffix=" m" color={COLORS.shaftColor} disabled={isAnimating} />
              <Slider label="Chamber Ø" value={chamberDia} onChange={(v) => { setChamberDia(v); if (shaftDia > v) setShaftDia(v); }} min={0.5} max={3.0} step={0.05} suffix=" m" color={COLORS.chamberColor} disabled={isAnimating} />
              ...
              <div className="space-y-2">
                <ResponsiveContainer width="100%" height={130}>
                  <AreaChart data={history} margin={{ top: 5, right: 10, bottom: 15, left: 10 }}>
                    ...
                    <Tooltip contentStyle={{ background: COLORS.panel, border: `1px solid ${COLORS.panelBorder}`, borderRadius: 6, fontSize: 10 }}
                      formatter={(val) => [val.toFixed(3) + " m", "WL Elev"]} />
                ...
                <ResponsiveContainer width="100%" height={100}>
                  <ComposedChart data={history} margin={{ top: 5, right: 10, bottom: 15, left: 10 }}>
                    ...
                    <Tooltip contentStyle={{ background: COLORS.panel, border: `1px solid ${COLORS.panelBorder}`, borderRadius: 6, fontSize: 10 }}
                      formatter={(val, name) => [val.toFixed(1) + " L/s", name === "inflow" ? "Inflow" : "Outflow"]} />
